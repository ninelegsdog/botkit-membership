# Testing guide — botkit-membership
...
`src/core/webhook.py` uses aiogram `SimpleRequestHandler(secret_token=...)` served by
aiohttp, which enforces the `X-Telegram-Bot-Api-Secret-Token` header.
`tests/test_webhook.py` asserts: valid secret → 200/202, wrong/missing secret → 401/403,
malformed JSON → 4xx/5xx.
